# *****************************************************************************
# Example Unity Connection and DQN 1-Human and 1-Agent Testing Script for Playing Unity Games
#
...
# **************************************************************************
# Initialize DQN and (s, a) state parameters
num_episods = 20	     # number of episodes used for testing
...
while True:
	...
	try:
		...
		h2_reward = 0			# reward for agent 1
		a1_episode_reward = 0	# total reward score for episode for agent 1
		h2_episode_reward = 0	# total reward score for episode for agent 2
		...
		while episode < num_episods+1:
			...
			if data:
				...
				h2_reward = h2_reward + data_int[7]-1
				done = data_int[8]
				...
				if done:
					...
					h2_episode_reward = h2_episode_reward+h2_reward 					# update episode a2 reward

					# Print and save current episode data
					print("Episode: {}, Frame Count: {}, Total A1 Reward: {}, Total H2 Reward: {}".format(episode, fcount, a1_episode_reward, h2_episode_reward))

					episode = episode+1													# increase episode count
					a1_episode_reward = 0												# rest total a1 reward for episode
					h2_episode_reward = 0												# rest total a2 reward for episode
					a1_reward = 0														# rest current a1 reward
					h2_reward = 0														# rest current a2 reward
					a1_action = 0														# set a1 action to 0
					message = "1 0 "													# set new outgoing message, with game rest
					...
					if fcount % pframe == 0:
						...
						message = "0 {} ".format(a1_action)								# set new outgoing message
						a1_reward = 0													# rest current reward
						h2_reward = 0													# rest current reward
